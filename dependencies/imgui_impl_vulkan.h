...
{
    VkInstance                      Instance; //device
    VkPhysicalDevice                PhysicalDevice; //device
    VkDevice                        Device; //device
    uint32_t                        QueueFamily; //adjacent to the swapchain? similar code was run in construction
    VkQueue                         Queue; //device.hpp by default has graphics & present queue
    VkDescriptorPool                DescriptorPool; //probably one of the sections we should create a specific allocation for               // See requirements in note above
    VkRenderPass                    RenderPass;                   // Ignored if using dynamic rendering
    uint32_t                        MinImageCount;                // >= 2
    uint32_t                        ImageCount; //FRAMES IN FLIGHT                  // >= MinImageCount
